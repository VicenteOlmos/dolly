...
Before starting a restore from the dump screen History section, the TUI MUST evaluate `clone.replace` and `clone.restore_on_conflict` from the loaded config. When `clone.replace` is true OR `restore_on_conflict` is `upsert`, the TUI MUST show a Y/N confirmation modal describing the destructive policy and target dump path. Enter/r on History MUST NOT start restore until the operator confirms with `Y`. `N` or Esc MUST dismiss without side effects. When policy is `error` or `skip` and `replace` is false, restore MUST proceed without a confirmation modal (existing Enter/r behavior).
